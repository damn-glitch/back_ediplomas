...
router.post(`/${prefix}/authorize-with-ds`, async (req, res) => {
    try {
        ...
        if (!user.rows.length) {
            ...
            if (existingIIN.rows.length) {
                console.log(existingIIN, "IIN found")
                let content_id = existingIIN.rows[0].content_id;
                await db.query(`update diplomas
                                set user_id = $1
                                where id = $2`, [user.rows[0].id, content_id])
